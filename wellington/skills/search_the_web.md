...
- `search_web` — web search: local index first (zero cost, instant), then a provider gateway (tavily → brave → searxng) on a miss. Parameters:
  - `query` (required — the exact parameter name).
  - `num_results` (optional, default 5; pass `10` for the full list).
  - `max_age_days` (optional — ignore locally-indexed pages crawled more than N days ago, forcing a live provider lookup; use for fast-moving topics).
